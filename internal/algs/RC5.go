...
func (rc5 *RC5) GenerateKeys(key []byte) ([][]byte, error) {
	if err := rc5.SetKey(key); err != nil {
		return nil, err
	}
	return nil, nil
}

func (rc5 *RC5) Encryption(inputBlock, roundKey []byte) ([]byte, error) {
	return rc5.Encrypt(inputBlock)
}

func (rc5 *RC5) Decryption(inputBlock, roundKey []byte) ([]byte, error) {
	return rc5.Decrypt(inputBlock)
}
